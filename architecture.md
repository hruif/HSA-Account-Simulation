...
- **The browser code has no automated tests, and the reason is cost, not that a browser cannot be tested.** jsdom with vitest would exercise `app.js` in milliseconds without a browser at all, and Playwright would drive the four flows end to end; a real product would carry both, because hand-checking a UI stops scaling after a few screens. Two things argue against them at this size. Almost every risk in `app.js` is a risk the server already refuses: a bad amount, an unknown category, another account's card or a missing cookie comes back 422, 404 or 401 whatever the page sends, and those paths are covered by pytest. And the price is a second toolchain, `package.json` and `node_modules` inside a Python submission, for 530 lines. What that leaves genuinely unchecked is the part no server test can reach: whether the card number really leaves the page's memory. Four places set `cardDetails` back to `null` — hide, log out, issue a card, change page — and only the code says so. That is the first thing to put under a jsdom test, and the reason the pure helpers were split into `static/format.js`: the split marks which half can be tested without a DOM at all.
